                            ...
                            <v-card-text>
                                <v-container>
                                    <v-row>
                                        ...
                                        <v-col cols="12" sm="6" md="4">
                                            <v-text-field v-model="editedItem.name2" label="Наименование"></v-text-field>
                                        </v-col>
                                        <v-col cols="12" sm="6" md="2">
                                            <v-text-field v-model="editedItem.koment" label="Комментарий"></v-text-field>
                                        </v-col>
                                    </v-row>
                                </v-container>
                            </v-card-text>
...
export default {
    data: () => ({
        ...
        headers: [
            { text: '№', value: 'id' },
            { text: 'Наиме-ние', value: 'name' },
            { text: 'Второе наиме-ние', value: 'name2' },
            { text: '', value: 'actions' },
        ],
        items: [],
        editedIndex: -1,
    ...
    methods: {
        initialize () {
            axios.post('/axios-send/workStatus',{status: 'all'}).then(respond => {
        ...
        deleteItemConfirm () {
            ...
            axios.post('/axios-send/workStatusDelete', {item: this.editedItem}).then(respond => {
                this.items = respond.data
            })
            this.closeDelete()
        },
        ...
        save (item) {
            ...
            this.close()
            axios.post('/axios-send/workStatusEditOrNew', {item: item}).then(respond => {
                this.items = respond.data
