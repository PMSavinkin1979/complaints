<template xmlns="http://www.w3.org/1999/html">
    <div>
        <v-container fluid class="pt-12">
            <v-container fluid>
                ...
                <v-card elevation="7" color="teal lighten-5">
                    <v-row dense class="pt-5 pb-0">
                        ...
                        <!--Год-->
                        <v-col cols="12" md="1" sm="6">
                            <v-text-field type="number" label="Год" v-model="god" hide-details
                                          @click="filterYear(god)"></v-text-field>
                        </v-col>
                        <!--Квартал-->
                        <v-col cols="12" md="3" sm="6">
                            <v-select style="font-size: xx-small"
                        ...
                        <v-col cols="12" md="2" sm="6">
                            <v-tooltip bottom>
                                <template v-slot:activator="{ on, attrs}">
                                    <v-checkbox label="все статусы" dense v-model="checkbox7" @change="check(checkbox7)"
                                                v-on="on"></v-checkbox>
                                </template>
                                <span>все статутсы за {{ god }}</span>
                            </v-tooltip>
                        </v-col>
                        <!--Пусто-->
                        <v-col cols="12" md="1" sm="6">
            ...
            <v-data-table dense :headers="headers" :items="danye" :search="search" :items-per-page="itemsPerPage"
                          class="elevation-7">
                <template v-slot:top>
                    <v-toolbar flat dense>
                        ...
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on, attrs}">
                                <v-checkbox class="pt-2" label="все записи" dense v-model="checkbox8"
                                            @change="check2(checkbox8)" v-on="on"></v-checkbox>
                            </template>
                                ...
                                <v-container fluid>
                                    <v-row dense>
                                        ...
                                        <v-col cols="12" md="2" class="pt-2">
                                            <v-tooltip bottom>
                                                <template v-slot:activator="{ on, attrs}">
                                                    <v-icon color="red" @click="close" v-bind="attrs" v-on="on">
                                                        mdi-close
                                                    </v-icon>
                                                </template>
                                        ...
                                        <v-col cols="12" md="2" class="pt-2">
                                            <v-tooltip bottom>
                                                <template v-slot:activator="{ on, attrs}">
                                                    <v-icon color="green" @click="save1" v-bind="attrs" v-on="on">
                                                        mdi-content-save
                                                    </v-icon>
                                                </template>
                                    ...
                                    <v-row dense>
                                        ...
                                        <v-col cols="12" md="3" class="">
                                            ...
                                            >
                                                ...
                                                <v-date-picker v-model="editedItem.data_end" no-title scrollable>
                                                    ...
                                                    <v-btn text color="primary" @click="$refs.menuEnd.save(date_end)">
                                                        OK
                                                    </v-btn>
                                                </v-date-picker>
                                            </v-menu>
                                        ...
                                        <!--приказ-->
                                        <v-col cols="12" md="3" class="">
                                            <v-text-field type="number" dense v-model="editedItem.prikaz"
                                                          label="Приказ"></v-text-field>
                                        </v-col>
                                        <!--№ Гарантийного приказ-->
                                        <v-col cols="12" md="3" class="">
                                            <v-text-field type="number" dense v-model="editedItem.prikaz_gar"
                                                          label="№ Гарантийного приказ"></v-text-field>
                                        </v-col>
                                        <!--Гарантия-->
                                        ...
                                        <!--вид надстройки-->
                                        <v-col cols="12" md="4" class="">
                                            <v-textarea rows="1" dense outlined v-model="editedItem.vid_ts"
                                                        label="Вид надстройки"></v-textarea>
                                        </v-col>
                                        <!--Причина гарантии-->
                                        <v-col cols="12" md="4" class="">
                                        ...
                                        <!--Заказчик-->
                                        <v-col cols="12" md="4" class="">
                                            <v-textarea rows="1" dense outlined v-model="editedItem.zakazchik"
                                                        label="Заказчик"></v-textarea>
                                        </v-col>
                                        ...
                                        <v-col cols="12" md="8" class="">
                                            <v-textarea rows="1" outlined dense v-model="editedItem.korrect"
                                                        label="Комментарий (мероприятия)"></v-textarea>
                                        </v-col>
                                            ...
                                            <v-tooltip bottom>
                                                <template v-slot:activator="{ on, attrs}">
                                                    <v-checkbox class="pt-0"
                                                                label="Документы перенаправленны виновной стороне" dense
                                                                v-model="editedItem.regressionBool"
                                            ...
                                            <v-tooltip top>
                                                <template v-slot:activator="{ on, attrs}">
                                                    <v-textarea dense rows="1" outlined v-model="editedItem.regression"
                                                                label="Перечень документов и описание"
                                                                v-on="on"></v-textarea>
                                                </template>
                                        ...
                                        <v-col cols="12" md="4" class="">
                                            <label>Затраты</label>
                                            <v-btn :disabled="disabled" color="primary" x-small @click="newPayment()">
                                                добавить
                                            </v-btn>
                                                ...
                                                <v-list nav dense>
                                                    <v-list-item-group v-model="paymentsItemsSelect" color="primary">
                                                        <v-list-item v-for="(item, i) in paymentsItems" :key="i">
                                                            <v-list-item-content v-on:mouseenter="show=true"
                                                            ...
                                                            <v-list-item-icon>
                                                                <v-icon small color="green"
                                                                        @click="editPayment(item.id, item.payment, item.months)">
                                                                    mdi-playlist-edit
                                                                </v-icon>
                                                                <v-icon small color="red"
                                                                        @click="deletePayment(item.id)">mdi-delete
                                                                </v-icon>
                                                            </v-list-item-icon>
                                                        </v-list-item>
                                                    </v-list-item-group>
                                        ...
                                        <v-dialog v-model="dialogPay" max-width="450">
                                            <v-card>
                                                        ...
                                                        <v-col cols="12" md="12">
                                                            <v-card elevation="7">
                                                                <v-card-text>
                                                                    ...
                                                                    <v-select v-model="monthSelect" :items="months"
                                                                              item-text="name" item-value="num"
                                                                              label="Период затрат" outlined
                                                                              dense></v-select>
                                                                    <!--затраты-->
                                                                    <v-text-field type="number" outlined dense
                                                                                  v-model="newZatraty"
                                                                                  label="Сумма затрат за период"></v-text-field>
                                                                </v-card-text>
                                                            </v-card>
                                                        </v-col>
                                                    </v-row>
                                                ...
                                                <v-card-actions>
                                                    <v-btn color="green darken-1" text @click="paymentsSave(idPayment)">
                                                        Сохранить
                                                    </v-btn>
                                        ...
                                        <!--выбор файлов-->
                                        <v-col cols="12" md="4" class="">
                                            <input style="display: none" type="file" id="files" ref="files" multiple
                                                ...
                                                <v-list dense>
                                                    <v-list-item-group color="primary">
                                                        <v-list-item v-for="(file, key) in oldFiles" :key="key">
                                                            <v-list-item-content
                                                                @click="readFile(key, file.title, file.mime)">
                                                                <v-list-item-title>
                                                                    {{ file.title }}
                                                                </v-list-item-title>
                                                            </v-list-item-content>
                                        ...
                                        <v-col cols="12" md="4" class="">
                                            <label>Добавить файлы
                                                <v-icon color="green" v-on:click="addFiles()">mdi-file-plus</v-icon>
                                            </label>
                                            <v-card>
                                                <v-list dense>
                                                    <v-list-item-group color="primary">
                                                        <v-list-item
                                                            ...
                                                            <v-list-item-content>
                                                                <v-list-item-title
                                                                    v-text="file.name"></v-list-item-title>
                                                            </v-list-item-content>
                                                            <v-list-item-icon>
                                                                <v-icon small color="red"
                                                                        v-on:click="removeFile( key )">mdi-file-remove
                                                                </v-icon>
                                                            </v-list-item-icon>
                                                        </v-list-item>
                                                    </v-list-item-group>
                        ...
                        <template v-slot:activator="{ on, attrs}">
                            <tr :class="getstatus(item.status)" @click="editItem(item)" v-on="on">
                                <td>
                                    <v-tooltip bottom>
                                        <template v-slot:activator="{ on, attrs}">
                                            <v-icon small left :color="iconColor(item)" v-on="on">{{
                                                    icon(item)
                                                }}
                                            </v-icon>
                                        </template>
...
export default {
    ...
    created() {
        if (localStorage.getItem('god') != null) {
            this.god = localStorage.getItem('god')
        } else {
            let date = new Date()
    ...
    methods: {
        initialize() { //
            ...
            axios.post('/axios-send/workStatus', {status: 'trimmed'}).then(respond => {
                ...
                let status = this.itemstatus.filter(function (items){
                    return items['name2'] == sta
                })
                this.itemStatusSelect = status[0]
            ...
            axios.post('/axios-send/garantiy', {status: 'trimmed'}).then(respond => {
                ...
                axios.post('/axios-send/statusChange', {
                    stat: sta,
        ...
        editItem(itemm) {
            ...
            if (this.editedItem.deleted_at != null) {
                this.itemStatusSelect_card = 'Удалена'
            }
            if (this.editedItem.flag == 1) {
                this.itemStatusSelect_card = 'В работе'
            }
            if (this.editedItem.flag == 2) {
                this.itemStatusSelect_card = 'Завершена'
            }
        ...
        deleteItem(itemm) {
            ...
            if (confirm('Вы уверенны в удалении записи?')) {
                axios.post('/axios-send/deleteZapis', {id: this.editedItem.id}).then(respond => {
                    ...
                    this.close();
                    axios.post('/axios-send/statusChange', {
                        stat: this.itemStatusSelect.name2,
                        months: this.monthSelect,
                        god: this.god
                    }).then(respond => {
                        this.danye = respond.data
        ...
        monthChange(months) {
            //console.log(months)
            axios.post('/axios-send/selectMonths', {
                stat: this.itemStatusSelect.name2, months: months,
                god: this.god, check: this.checkbox7
            }).then(respond => {
                this.danye = respond.data
                //this.zatratyAll = this.totalSum
                //this.summaZatraty()
        ...
        check(stat) {
            ...
            if (this.monthSelect.length != 0) {
                axios.post('/axios-send/selectMonths', {
                    stat: this.itemStatusSelect.name2, months: this.monthSelect,
                    god: this.god, check: this.checkbox7
                }).then(respond => {
                    this.danye = respond.data
                    //this.zatratyAll = this.totalSum
        ...
        paymentsSave(idPayment) {
            axios.post('/axios-send/paymentsSave', {
                id: this.editedItem.id, zatraty: this.newZatraty,
                months: this.monthSelect, idPayment: idPayment, god: this.god
            }).then(respond => {
                this.paymentsItems = respond.data
            })
